...
var goUnix = function(ret, cb) {
  ...
  if (!alreadyLinked && !ret["where_jx"].err && ret["where_jx"]) {
    ...
    for (var o in arr) {
      ...
      var jxFromPATH = arr[o].trim();
      if (!fs.existsSync(jxFromPATH)) {
        console.error("jxFromPATH does not exist?");
        continue;
      }

      // reading info about previously installed jx
      common.readBinaryInfo(jxFromPATH, function (info) {
        ...
        var localJX = path.join(input.localDir, path.basename(jxFromPATH));
        var copied = common.copySync(jxFromPATH, localJX);
        ...
        var copied = common.copySync(jxFromPATH, targetJX);
        ...
        if (cnt === len)
          return cb()
        //console.info("jxFromPATH", jxFromPATH, info);
      });
    }
  } else {
    // jx was not found
