...
var getName = function() {
  return common.parsedArgv.init.value;
};
...
var goUnix = function (ret, cb) {
  ...
  var _finish = function (createLink, input, skip) {
    ...
    if (createLink) {
      //console.log("linking", targetJX, newName);
      //console.log("arr", arr);
      var linked = link(targetJX, newName);
      if (linked.err)
...
exports.run = function (input, cb) {
  ...
  if (!fs.existsSync(common.dirVersions)) {
    try {
      ...
      return common.logErrorAndExit("Cannot create directory:", common.dirVersions, "\n", ex)
    }

    try {
      fs.chmodSync(common.dirVersions, 0777);
    } catch(ex) {
      return common.logErrorAndExit("Cannot chmod directory:", common.dirVersions, "\n", ex)
    }
  }
